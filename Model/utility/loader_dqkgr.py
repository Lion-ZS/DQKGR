...
class DQKGR_loader(Data):
    def __init__(self, args, path):
        ...
        # generate the sparse laplacian matrices for each matrix in self.adj_list
        # i.e, transform the orign self.adj_list to self.lap_list
    ...
    def _get_relational_adj_list(self):
        ...
        # "0" represent user-item interacted relation
        adj_r_list.append(0)

        adj_mat_list.append(R_inv)
        
        # "n_relations + 1" represent item-user interaction
        ...
        for r_id in self.relation_dict.keys():
            ...
            # each "relation_id + 1" represent the head-tail relation
            adj_r_list.append(r_id + 1)

            adj_mat_list.append(K_inv)
            # each "rid + 1 + n_relation + 1" represent the tail-head relation
    ...
    def _get_relational_lap_list(self):
        # two different type of laplacian matrix
    ...
    def _get_all_kg_dict(self):
        # lzp: merge U-I interaction and KG into the whole KG dict  {h: [t,r]}
    ...
    def _get_all_kg_data(self):
        # lzp: return four list of the whole U-I & KG data [h_1, ..., h_n], [r_1, ..., r_n], [t_1, ..., t_n],[v_1, ..., v_n]
        ...
        # the sorted version of org_h_dict
        sorted_h_dict = dict()
        ...
        # lzp: OrderdDict: popitem(), move_to_end()
    ...
    def _generate_train_A_batch(self, n_batch):
        exist_heads = self.all_kg_dict.keys()
        # add by lzp
        self.batch_size_kg = int(len(self.all_h_list)/n_batch)
    ...
    def generate_train_batch(self, n_batch):
        users, pos_items, neg_items = self._generate_train_cf_batch()
        
        # add by lzp
        heads, relations, pos_tails, neg_tails = self._generate_train_A_batch(n_batch)
    ...
    def generate_train_A_batch(self, n_batch):
        heads, relations, pos_tails, neg_tails = self._generate_train_A_batch(n_batch)
