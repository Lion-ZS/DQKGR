...
from utility.loader_qkgn import QKGN_loader
...
if args.model_type in ['qkgn']:
    data_generator = QKGN_loader(args=args, path=args.data_path + args.dataset)
...
def ranklist_by_heapq(user_pos_test, test_items, rating, Ks):
    ...
    r = []
    # record the result of top items
...
def test_one_user(x):
    ...
    # in kgat & qkgn, args.test_flag = 'part'(default)
    if args.test_flag == 'part':
        r, auc = ranklist_by_heapq(user_pos_test, test_items, rating, Ks)
        # lzp: in ranklist_by_heapq: auc = 0. , but i change it to add auc
    else:
        r, auc = ranklist_by_sorted(user_pos_test, test_items, rating, Ks)
        # lzp: in ranklist_by_sorted: auc = get_auc(item_score, user_pos_test).
...
def test(sess, model, users_to_test, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        # in kgat & qkgn, batch_test_flag = False
        if batch_test_flag:
